// Embedded mpv render-API integration for macOS.
//
// Adapted from IPTVnator (https://github.com/4gray/iptvnator),
// apps/electron-backend/native/src/embedded_mpv.mm — MIT License,
// Copyright (c) 4gray and IPTVnator contributors. Retained here under MIT.
//
// Renders libmpv frames (vo=libmpv + mpv_render_context, OpenGL) into an
// app-owned NSOpenGLView that is a SUBVIEW of the Electron window's content
// view, so the video is truly embedded (no separate/--wid window). The host
// container view returns nil from hitTest: so mouse events pass through to
// Chromium and the HTML controls stay interactive.
...
Napi::Value CreateSession(const Napi::CallbackInfo& info)
{
    ...
    // Seeking/buffering robustness for HTTP VOD (MKV/MP4): a seekable demuxer
    // cache lets scrubs land within already-downloaded data without re-opening
    // the stream, force-seekable makes range-capable HTTP VOD scrub even when
    // the server under-advertises, and the network reconnect/timeout options
    // recover dropped live connections instead of erroring out.
    mpv_set_option_string(session->handle, "cache", "yes");
    mpv_set_option_string(session->handle, "cache-secs", "60");
    mpv_set_option_string(session->handle, "demuxer-max-bytes", "128MiB");
    mpv_set_option_string(session->handle, "demuxer-max-back-bytes", "64MiB");
    mpv_set_option_string(session->handle, "demuxer-readahead-secs", "20");
    mpv_set_option_string(session->handle, "demuxer-seekable-cache", "yes");
    mpv_set_option_string(session->handle, "force-seekable", "yes");
    mpv_set_option_string(session->handle, "network-timeout", "15");
    mpv_set_option_string(session->handle, "stream-lavf-o",
...
Napi::Value Seek(const Napi::CallbackInfo& info)
{
    ...
    // Keyframe seek (NOT exact): exact seeks decode every frame from the prior
    // keyframe to the target, which lags badly over HTTP and provokes MKV
    // "no join point found" demux warnings on multi-stream files. Keyframe seeks
    // are near-instant and land close enough for VOD scrubbing.
    const char* command[] = {
        "seek",
        targetValue.c_str(),
        "absolute+keyframes",
